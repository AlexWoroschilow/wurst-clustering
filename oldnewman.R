...
do.clustering<-function(graph, clustering, depth){
	sidx <- 1
    clid <- clustering[length(clustering[,1]), 3]
    depth <- depth+1;
	if ( length(V(graph)) < 5 ){    # Simply add trivial clusters to output
        ...
        # if the graph is compact, contains only one cluster or is too deep, add clusters to the solution
		if(diameter(graph, directed=FALSE, weights=NA) < 4 || length(unique(ms)) < 2 || depth > 2){
			...
			for (i in sort(unique(ms))){
                ...
                rm(sgids)
			    # cluster the subgraph (RECURSION HERE)	
				tc<-do.clustering(sg, clustering, depth)
                rm(sg)
				clustering<-rbind(clustering, tc)
                rm(tc)
			}
...
#	clustering <- data.frame(ID=rep(" ", length(V(G))), DEG=rep(NA, length(V(G))), CLUST=rep(NA, length(V(G))), 	stringsAsFactors=FALSE)
	clustering <- data.frame(ID = " ", DEG=0, CLUST=0, stringsAsFactors=FALSE)
	clid <- 0;
	sidx <- 1;
...
#	for ( c in 1:3){
		sidx <- 1;
        cat("graph ", c, '\n')
		do.clustering(cls2[[c]], clustering, 0)
	}
...
print.clustering(G, filen="/home/margraf/rstuff/tm_recursive_0.4.clust")
